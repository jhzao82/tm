...
SELECT p.*, COALESCE(p.chinese_name, full_name(p.first_name, p.last_name)) AS name, regexp_replace(initcap(first_name), '[^A-Z\s]+', '.', 'g') || ' ' || last_name AS short_name, age(dob) AS age,
